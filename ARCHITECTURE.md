...
## 2. Problem Statement
Historically, orchestration platforms tightly couple workflow semantics (DAG resolution, state machines) with task execution (queues, workers, polling). This coupling leads to complex scaling bottlenecks, rigid deployment models, and monolithic codebases. We need a system where the orchestrator manages *what* happens and *when*, while a specialized execution platform handles *how* to reliably execute arbitrary work under distributed failure conditions.
