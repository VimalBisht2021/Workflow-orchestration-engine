...
export default function RunDetailsPage() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/workflow-runs/${id}`)
      .then(res => res.json())
      .then(data => setRun(data))
      .catch(console.error);

    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/workflow-runs/${id}/tasks`)
      .then(res => res.json())
      .then(data => setTasks(data))
      .catch(console.error);

    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/workflow-runs/${id}/events`)
      .then(res => res.json())
