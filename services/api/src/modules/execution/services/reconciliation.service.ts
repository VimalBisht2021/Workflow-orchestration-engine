...
import { TaskCompletedDomainEvent, TaskFailedDomainEvent } from '../events/domain/domain-events';
...
export class ReconciliationService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    this.executionClient = new ExecutionClient({
      ...
      apiKey: this.configService.get<string>('DTP_API_KEY', 'default-key'),
  ...
  onModuleInit() {
    ...
    this.timer = setInterval(() => {
      this.reconcileStrandedTasks().catch(err => {
        this.logger.error(`Unhandled error in reconcileStrandedTasks: ${err.message}`);
      });
    }, intervalMs);
  }
    ...
    try {
      ...
      for (const task of strandedTasks) {
        try {
          ...
          const jobStatus = await this.executionClient.getJobStatus(idempotencyKey);
          
          if (!jobStatus) {
            this.logger.warn(`Job for TaskRun ${task.id} not found in DTP. Marking as failed.`);
            await this.markTaskFailed(task.id, task.workflowRunId, 'Job not found in DTP during reconciliation');
            continue;
          }
          ...
          if (status === 'COMPLETED') {
            ...
            await this.markTaskFailed(task.id, task.workflowRunId, errorMsg, new Date(jobStatus.updatedAt || Date.now()));
          }
        } catch (e: any) {
          this.logger.error(`Error reconciling TaskRun ${task.id}: ${e.message}`);
        }
      }
    } catch (e: any) {
  ...
  private async markTaskFailed(taskRunId: string, workflowRunId: string, error: string, completedAt = new Date()) {
    await this.prisma.taskRun.update({
      where: { id: taskRunId },
      data: {
        status: 'FAILED',
